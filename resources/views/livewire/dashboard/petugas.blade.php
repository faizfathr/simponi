<div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 "x-data="{
    ...
    <div x-show="openWarningDelete"
        ...
        <x-utility.delete-confirmator show="openWarningDelete" title="Hapus Petugas" description="Anda yakin ingin menghapus petugas <b>{{ $this->nama ?? '' }}</b> ?">
            <x-slot:action>
                <button wire:click="deletePetugas('{{ $this->id }}')"
                    ...
                    Hapus
                    <x-utility.loader target="deletePetugas" />
                </button>
            </x-slot:action>
    ...
    <div x-show="openForm" x-transition x-cloak
        ...
        <div @click.outside="openForm = false"
            ...
            <form wire:submit.prevent="simpanData" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {{-- NIP --}}
                <div>
                    <label for="nip" class="block mb-2 font-semibold text-gray-700 dark:text-gray-300">NIP</label>
                    <input type="text" id="nip" wire:model.defer="nip"
                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 focus:ring focus:ring-blue-300 px-5 py-3 text-gray-900 dark:text-white">
                    @error('nip')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div >
                    <label for="jabatan"
                        class="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Jabatan</label>
                ...
                <div>
                    <label for="golongan"
                        class="block mb-2 font-semibold text-gray-700 dark:text-gray-300">golongan</label>
                    <select wire:model.defer="golongan"
                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 focus:ring focus:ring-blue-300 px-5 py-3 text-gray-900 dark:text-white">
                        <option value="">Pilih golongan</option>
                        @foreach ($listGolongan as $item)
    ...
    <table class="w-full mt-4 text-left text-sm text-gray-900 dark:text-gray-200">
        <thead class="bg-gray-100 dark:bg-gray-700">
            <tr>
                ...
                <th class="py-3 px-5">NIP</th>
                <th class="py-3 px-5">Nama</th>
                <th class="py-3 px-5">Jabatan</th>
                <th class="py-3 px-5">Golongan/Pangkat</th>
        ...
        <tbody>
            @forelse ($listPetugas as $item)
                <tr
                    ...
                    <td class="py-3 px-5">{{ $item->nip ?? '-' }}</td>
                    <td class="py-3 px-5">{{ $item->nama }}</td>
                    <td class="py-3 px-5">{{ $item->getJabatan->nama_jabatan ?? 'Mitra Statistik' }}</td>
                    <td class="py-3 px-5">{{ $item->getGolongan->nama_golongan ?? '-' }}</td>
                    <td class="py-3 px-5">{{ $item->no_rek }}</td>
                    <td class="py-3 px-5">{{ $item->status === 0 ? 'Organik' : 'Non Organik' }}</td>
                    <td class="py-3 px-5">
                        @php
                    ...
                    <td>
                        <button wire:click="editPetugas('{{ $item->id }}')"
                        ...
                        <button wire:click="confirmDelete('{{ $item->id }}')"
                            ...
                            <div wire:loading.remove wire:target="confirmDelete('{{ $item->id }}')" class="flex items-center gap-1">
                                <x-icons.trash />
                                Hapus
                            </div>
                            <x-utility.loader target="confirmDelete('{{ $item->id }}')" />
                        </button>
                    </td>
