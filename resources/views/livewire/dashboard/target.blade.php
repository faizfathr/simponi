<div x-data="{
    ...
    <div x-show="openWarningDelete"
        class="space-y-6 fixed inset-0 flex items-center justify-center bg-black/50 z-50 overflow-scroll scrollbar-hide">
        <x-utility.delete-confirmator show="openWarningDelete" title="Hapus Target Kegiatan" description="Anda yakin ingin menghapus target kegiatan <b>{{ $this->kegiatan ?? '' }}</b> di {{ $this->periode ?? '' }} {{ $this->ketWaktu ?? '' }} ?">
            <x-slot:action>
