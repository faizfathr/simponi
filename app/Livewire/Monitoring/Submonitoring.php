...
class Submonitoring extends Component
{
    ...
    public function render()
    {
        $contentsYearly = ListKegiatan::leftJoin('kegiatan_survei', 'id_kegiatan', 'kegiatan_survei.id')
            ...
            ->selectRaw('list_kegiatan.id_kegiatan, kegiatan, subsektor, target, periode, list_kegiatan.tahun as tahun, list_kegiatan.tanggal_mulai as mulai, list_kegiatan.tanggal_selesai as selesai, Count(status) as realisasi')
            ->groupBy('list_kegiatan.id_kegiatan', 'kegiatan', 'target', 'subsektor', 'periode', 'mulai', 'selesai', 'list_kegiatan.tahun')
            ->where('subsektor', $this->idMonitoring)
            ->where('list_kegiatan.tahun', now()->year)
            ->where('periode', 4)
            ->get();
